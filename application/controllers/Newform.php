...
// form management
 public function viewForms(){
	$this->load->model('formmodel');
	$data['form_name'] = $this->formmodel->selectForms();
	$this->load->view('dynamicform/showForms',$data);
 }
 ...
 public function addStudentsForEvent(){
	...
	$this->load->model('formmodel');
	$form_title =$this->formmodel->getFormName($form_id);
	$data['form_name'] = $form_title;
	$data['form_id'] = $form_id;

	
	$data['form_field_result'] =$this->formmodel->getFormFields($form_id);
	$this->load->view('dynamicform/viewTheSelectedForm',$data);
...
public function deleteForm(){
	$form_id= $this->uri->segment(3);
	$this->load->model('formmodel');
	 $deleted = $this->formmodel->deleteForm($form_id);
	 if($deleted){
		$this->session->set_flashdata('formDeleted', '<div class="alert alert-success  text-center">  Successfully Deleted The Form.....</div>');
		redirect('/newform/viewForms/');
	 }
}

// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

//view all option view-------------------------------
public function changeAllOptionView(){
	  
    $this->load->model('formmodel');
     $options['textFields'] = $this->formmodel->getTextFields();
     $options['dropdown'] =$this->formmodel->getDropDowns();
     $options['checkbox'] =$this->formmodel->getCheckBoxes();
     $options['titletext'] =$this->formmodel->getExistingTextFields();
   
	if($options){
		$this->load->view('dynamicform/allOptionView',$options);
	}else{
...
// add all option view----------------------------------
public function saveNewAllOptions($form_element_id, $name){

    $data = array('form_element_id' => $form_element_id, 'name'=> $name);
    $this->load->view('dynamicform/addAllOptionView',$data);

}
// ends here-----------------------------------------------------
// save extraallfields -------------------------
public function saveAllExtraField(){
    $this->load->model('formmodel');
