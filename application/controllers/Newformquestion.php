...
class Newformquestion extends CI_Controller {
	...
	$this->session->set_flashdata('inserted', '<div class="alert alert-success  text-center">  Successfully Created The Questionnaire.....</div>');
...
public function viewForms(){
	$this->load->model('formmodelquestion');
	$data['form_name'] = $this->formmodelquestion->selectForms();
	$this->load->view('dynamicformquestion/showForms',$data);
 }
 ...
 public function addStudentsForEvent(){
	...
	$this->load->model('formmodelquestion');
	$form_title =$this->formmodelquestion->getFormName($form_id);
	$data['form_name'] = $form_title;
	$data['form_id'] = $form_id;

	
	$data['form_field_result'] =$this->formmodelquestion->getFormFields($form_id);
	$this->load->view('dynamicformquestion/viewTheSelectedForm',$data);
...
public function deleteForm(){
	$form_id= $this->uri->segment(3);
	$this->load->model('formmodelquestion');
	 $deleted = $this->formmodelquestion->deleteForm($form_id);
	 if($deleted){
		$this->session->set_flashdata('formDeleted', '<div class="alert alert-success  text-center">  Successfully Deleted The Questionnaire.....</div>');
		redirect('/newformquestion/viewForms/');
	 }
}
